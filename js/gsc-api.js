...
// Uses fetch() directly with the Bearer token from Google Identity Services OAuth.
...
function gscFetch(path, method, body, callback) {
  ...
  fetch(GSC_API + path, opts)
    .then(function(r) {
      if (r.status === 401) { S.accessToken = null; S.gscStatus = 'disconnected'; render(); throw new Error('Token expirado — vuelve a conectar'); }
